...
Draft 2021-12-16: Added clarifications and more background, explanations, wording clarifications, references, summarized performance study, applications, more questions/answers. --Based on feedback and material sent to SG9 list 2021-12-09. Synopsis: \tcode{S last} argument was missing 2021-12-19.
...
The tony table with this proposal, \emph{After}, for all examples are the same:\newline
...
A small sorted windows into a large data set can be selected as if sorted by partitioning at two points. For example, if `j` items fit on a display page, we can jump to page \tcode{k}, that is, into the range from \tcode{a=v.begin()+j*k} to \tcode{b=v.begin()+j*(k+1)}:
...
%\includegraphics[width=0.50\textwidth]{plotting/examples/forsen.png} &
%\includegraphics[width=0.50\textwidth]{plotting/examples/forsen_sort.png} \\
%\hline
%Equalized using range-of-nths: m=3: & Equalized using range-of-nths: m=3\\
%\includegraphics[width=0.50\textwidth]{plotting/examples/forsen_partition2.png} &
%\includegraphics[width=0.50\textwidth]{plotting/examples/forsen_partition5.png}\\
%\hline
%\end{tabular}
%
...
%The exact CDF (using sort) is shown in gray, along with the `range-of-nths` approximations.
%
%\includegraphics[width=0.21\textwidth]{plotting/examples/forsen_cdf_approximation.png}
%
